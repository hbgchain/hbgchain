...
const string strMessageMagic = "HBGChain Signed Message:\n";
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "August 15, 2017. The HBGChain release. ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HBGChainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HBGChainMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("HBGChainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("HBGChain-miner");
    ...
    try { loop {
        ...
        printf("Running HBGChainMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("HBGChainMiner terminated\n");
        throw;
    }
}
